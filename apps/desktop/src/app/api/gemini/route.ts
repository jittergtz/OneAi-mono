          ...
          try {
            ...
            for await (const chunk of result.stream) {
              ...
              if (useSearch && searchChunk.tool_calls) {
                console.log(
                  "\n--- Tool Calls (Grounding/Search Results) - CHUNK with tool_calls ---"
                )
                      ...
                      if (searchResults && searchResults.results) {
                        searchResults.results.forEach(
                          (searchResult: any, index: number) => {
                            ...
                            // Add to sourceLinks if not already present
                            const exists = sourceLinks.some(link => link.url === searchResult.url);
                            if (!exists) {
                              sourceLinks.push({
            ...
            // Add final metadata object with the complete text and sources
            console.log("Adding sources to response:", sourceLinks)
            controller.enqueue(
            ...
            console.log("Controller errored DUE TO STREAM ERROR.")
          }
        },
      })
